          ...
          listener: (context,state){
            if(state is OrderLoaded){
              if(state.status==Status.failure){
                ...
                EasyLoading.showSuccess('Berhasil memberi rating');
                _orderBloc.add(FetchOrderEvent(widget.status,start: state
                    .start,end: state.end));
              }
            }
          },
              ...
              return SingleChildScrollView(
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                  widget.status!='selesai'?const SizedBox.shrink():
                      state.start.isEmpty
                          ? ActionChip(
                          ...
                          : Chip(
                              label: Text(
                                  '${getDateDashboard(DateTime.parse(state.start))} - ${getDateDashboard(DateTime.parse(state.end))}'),
                              onDeleted: () {
                                _orderBloc.add(FetchOrderEvent(widget.status));
                              },
                            ),
                          ...
                          ? Card(
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  children: [
                                    Text(
                                      'Bagaimana tanggapan anda tentang restoran '
                                      'kami?',
                                                          ...
                                                          BorderRadius.vertical(
                                                              top: Radius
                                                                  .circular(
                                                                      20))),
                                              builder: (context) =>
                                          DialogRating(idUser:state.idUser ,
                      ...
                      state.data.isEmpty
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return ListTile(
                                  title: Text(
                                      '${order.tipe} (${order.detailOrder.length} item)'),
                                  subtitle: Column(
                                    crossAxisAlignment:
                                        CrossAxisAlignment.start,
                                    children: [
                                      Text(
                                        valueRupiah(order.total),
                                        style: blackTextStyle.copyWith(
                                            fontWeight: bold),
                                      ),
                                      Text(
                                          '${order.jam.substring(0, 5)} / ${getDateDashboard(order.tanggal)}'),
                                    ],
                                  ),
                                  trailing: Chip(
                                    label: Text(
                                      order.statusOrder[0].status,
                                  ...
                                  onTap: () {
                                    Navigator.pushNamed(
                                        context, RouteList.detailOrderAdmin,
                                        arguments: order.id);
                                  },
                                );
                              },
        ...
        child: Column(
          ...
          children: [
            TextFieldWidget(
              ...
              readonly: true,
              validator: (value) =>
                  FormValidation.validate(value ?? '', label: 'Tanggal mulai'),
              onTap: () {
                showDatePicker(
                        context: context,
            ...
            TextFieldWidget(
              ...
              controller: end,
              readonly: true,
              validator: (value) => FormValidation.validate(value ?? '',
                  label: 'Tanggal selesai'),
              onTap: () {
                showDatePicker(
                        context: context,
                        initialDate: start.text.isNotEmpty
                            ? DateTime.parse(start.text)
                            : DateTime.now(),
                        firstDate: start.text.isNotEmpty
                            ? DateTime.parse(start.text)
                            : DateTime.now(),
                        lastDate: DateTime.now().add(const Duration(days: 365)))
                    .then((value) {
            ...
            CustomFlatButton(
              ...
              onPressed: () {
                if (_form.currentState?.validate() ?? false) {
                  if (DateTime.parse(start.text).isBefore(
                      DateTime.parse(end.text).add(Duration(days: 1)))) {
                    Navigator.pop(
...
class DialogRating extends StatefulWidget {
  ...
  const DialogRating({Key? key, required this.idUser, required this.orderBloc}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        MainAxisSize.min,
        children: [
          Text('Beri Rating',style: blackTextStyle.copyWith(fontWeight: bold),),
          vSpace(10),
          RatingBar.builder(
            initialRating: 5,
            minRating: 1,
          ...
          vSpace(10),
          CustomFlatButton(backgroundColor: AppColor.primary, label: 'SIMPAN'
              , onPressed: (){
            Navigator.pop(context);
            widget.orderBloc.add(PostRatingEvent
              (RatingParams(widget.idUser, rating.toString(),catatan: catatan
                    .text)));
              }),
          Padding(
