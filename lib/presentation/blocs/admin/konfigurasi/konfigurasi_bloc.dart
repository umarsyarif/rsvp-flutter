...
import 'package:kopiek_resto/domain/usecases/order/check_seat.dart';
...
class KonfigurasiBloc extends Bloc<KonfigurasiEvent, KonfigurasiState> {
  ...
  final CheckSeat checkSeat;

  KonfigurasiBloc(this.konfigurasi, this.logout, this.loadingBloc,
      this.updateKonfigurasi, this.checkSeat) : super(KonfigurasiInitial()) {
    on<FetchKonfigurasiEvent>((event, emit) async {
      emit(KonfigurasiLoading());
      final eith = await konfigurasi.call(NoParams());
      eith.fold((l) => emit(KonfigurasiFailure(l.message)), (r) =>
          emit(KonfigurasiLoaded(r)));
    });
    on<LogoutEvent>((event, emit) async {
      emit(KonfigurasiLoading());
      final eith = await logout.call(NoParams());
      eith.fold((l) => emit(KonfigurasiFailure(l.message)), (r) =>
          emit(KonfigurasiLogout()));
    });
      ...
      if (currentState is KonfigurasiLoaded) {
        ...
        eith.fold((l) =>
            emit(currentState.copyWith(
                status: Status.failure, errMessage: l.message)), (r) =>
    ...
    on<UbahKapasitasRestoranEvent>((event, emit) async {
      final currentState = state;
      if (currentState is KonfigurasiLoaded) {
        emit(currentState.copyWith(status: Status.loading));
        loadingBloc.add(StartLoading());
        final eith = await updateKonfigurasi.call({
          'id': 1,
          'jumlah_kursi': event.kapasitasRestoran
        });
        eith.fold((l) =>
            emit(currentState.copyWith(
                status: Status.failure, errMessage: l.message)), (r) =>
    ...
    on<CheckSeatEvent>((event, emit) async {
      final currentState = state;
      if (currentState is KonfigurasiLoaded && event.date.isNotEmpty && event
          .jumlahOrang.isNotEmpty) {
        final eithCheckSeat = await checkSeat.call(CheckSeatParams(event.date,
            int.parse(event.jumlahOrang)));
        emit(eithCheckSeat.fold((l) =>
            currentState.copyWith(seatCapacity: SeatCapacity.full), (r) =>
            currentState.copyWith(
                seatCapacity: r ? SeatCapacity.available : SeatCapacity.full)));
      }
    });
  }
}
